...
def test_smile_activates_happy():
    names, scores = _scores(
        mouthSmileLeft=0.9, mouthSmileRight=0.9, cheekSquintLeft=0.5, cheekSquintRight=0.5
    )
    label, score = classify_expression(names, scores)
    assert label == "Happy"
...
def test_result_is_always_one_of_the_defined_categories():
    ...
    names, scores = _scores(
        mouthSmileLeft=1.0, mouthSmileRight=1.0, mouthFrownLeft=1.0, mouthFrownRight=1.0
    )
    label, score = classify_expression(names, scores)
