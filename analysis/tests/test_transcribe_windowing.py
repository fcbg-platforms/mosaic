...
def test_segment_well_before_margin_is_confirmed():
    ...
    confirmed, tentative, watermark = confirm_segments(
        segs, buffer_duration_sec=5.0, trailing_margin_sec=1.0
    )
...
def test_empty_segments_yields_no_confirmation():
    confirmed, tentative, watermark = confirm_segments(
        [], buffer_duration_sec=3.0, trailing_margin_sec=1.0
    )
...
def test_boundary_exactly_at_margin_is_confirmable():
    ...
    confirmed, _, watermark = confirm_segments(
        segs, buffer_duration_sec=5.0, trailing_margin_sec=1.0
    )
...
def test_multiple_segments_confirm_in_one_pass_after_a_pause():
    ...
    confirmed, tentative, watermark = confirm_segments(
        segs, buffer_duration_sec=4.5, trailing_margin_sec=1.0
    )
