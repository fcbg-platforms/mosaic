...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument("--audio",   metavar="FILE",
                        help="Transcribe/diarize a single audio file")

    parser.add_argument("--model", choices=["tiny", "base", "small", "medium", "large-v3"],
                         default="small", help="faster-whisper model size (default: small)")
    parser.add_argument("--language", default=None,
                         help="Force a language code (e.g. 'en'); default auto-detects")
    parser.add_argument("--device", default=None,
                         help="cpu / cuda — default auto-selects cuda if available")
    ...
    parser.add_argument("--min-speakers", type=int, default=0,
                         help="Optional hint for pyannote (0 = unset)")
...
def _load_models(model_size: str, device: Optional[str], hf_token: Optional[str],
    ...
    print(f"[run_diarize] Loading whisper model (model={model_size}, "
          f"device={resolved_device})...", flush=True)
    whisper_model = load_whisper_model(model_size, resolved_device)

    diarization_pipeline = None
    if skip_diarization:
        print("[run_diarize] Skipping diarization: --skip-diarization requested "
              "— transcript only.", flush=True)
    elif not token:
        print("[run_diarize] Skipping diarization: no Hugging Face token provided "
              "— transcript only. Pass --hf-token or set HF_TOKEN to enable "
              "speaker labels.", flush=True)
    else:
...
def process_session(session_dir: Path, model_size: str, language: Optional[str],
                     device: Optional[str], hf_token: Optional[str],
                     min_speakers: int, max_speakers: int, skip_diarization: bool) -> None:
    ...
    for audio_path in audios:
        ...
        if not process_audio(audio_path, out_path, model_size, whisper_model,
                              diarization_pipeline, language, min_speakers, max_speakers):
            failures += 1

    if failures:
        print(f"[run_diarize] {failures}/{len(audios)} file(s) failed to process "
              f"— see errors above.", file=sys.stderr)
        sys.exit(1)


def process_audio(audio_path: Path, out_path: Path, model_size: str, whisper_model,
                   diarization_pipeline, language: Optional[str],
                   min_speakers: int, max_speakers: int) -> bool:
    ...
    try:
        whisper_segments, detected_language = transcribe_audio(
            whisper_model, audio_path, language)
    ...
    if diarization_pipeline is None:
        print(f"[run_diarize] No diarization pipeline available for {audio_path.name} "
              f"— transcript only.", flush=True)
    else:
    ...
    print(f"[run_diarize] Done. {len(segments)} segment(s) in {elapsed:.1f}s → {out_path}",
          flush=True)
    return True
...
def main() -> None:
    ...
    if args.session:
        process_session(Path(args.session), args.model, args.language, args.device,
        ...
        if not process_audio(audio_path, out_path, args.model, whisper_model,
                              diarization_pipeline, args.language,
