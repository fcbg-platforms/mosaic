...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument("--session", metavar="DIR",
                        help="Anonymize all .mp4 files in a recorded session directory")
    group.add_argument("--video",   metavar="FILE",
                        help="Anonymize a single video file")

    parser.add_argument("--backend", choices=["mediapipe", "yolov8", "opencv"],
                         default="mediapipe",
                         help="Face-detection backend (default: mediapipe)")
    parser.add_argument("--model", default=None,
                         help="Model override (only meaningful for --backend yolov8)")
    parser.add_argument("--style", choices=["blur", "box"], default="blur",
    ...
    parser.add_argument("--device", default=None,
                         help="Inference device for the yolov8 backend: cpu / cuda:0")
    return parser.parse_args()

# ── Session mode ─────────────────────────────────────────────────────────────

def process_session(session_dir: Path, backend: str, model: Optional[str],
                     style: str, conf: float, skip: int, device: Optional[str]) -> None:
    ...
    if failures:
        print(f"[run_face_mask] {failures}/{len(videos)} video(s) failed to process "
...
def process_video(video_path: Path, detector, out_path: Path, style: str, skip: int) -> bool:
    ...
    if frame_idx == 0:
        print(f"[run_face_mask] {video_path.name}: 0 frames read — removing empty output",
              file=sys.stderr)
...
def main() -> None:
    ...
    if args.session:
        process_session(Path(args.session), args.backend, args.model,
                         args.style, args.conf, args.skip, args.device)
    elif args.video:
