...
bool WavWriter::open(const QString& path, int sampleRate, int channels, int bitsPerSample) {
    ...
    if (!d->file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        log_error(QString("[WavWriter] Cannot open: %1 — %2").arg(path, d->file.errorString()));
        return false;
    }
...
void WavWriter::close() {
    ...
    log_info(
        QString("[WavWriter] Closed — %1 bytes, %2 s")
            .arg(d->bytesWritten)
            .arg(double(d->bytesWritten) / (d->sampleRate * d->channels * d->bitsPerSample / 8), 0,
                 'f', 2));
}
