...
uint32_t ipv4_broadcast_address(uint32_t ip, uint32_t mask) { return ip | ~mask; }
...
double action_command_period_ms(const std::vector<double>& targetFps, double marginFactor) {
    ...
    if (!std::isfinite(minFps)) {
        minFps = k_default_action_fps;
    }
    if (!(marginFactor > 0.0 && marginFactor <= 1.0)) {
        marginFactor = 1.0;
    }
...
bool is_achievable_fps_measurement_warmed_up(double secondsSinceGrabbingStarted,
    ...
    if (secondsSinceGrabbingStarted < 0.0) {
        return false;
    }
...
struct ActionCommandSession::Impl {
    ...
    Impl() {
        try {
            ...
            if (!gigeTl) {
                log_error(
                    "[ActionCommand] GigE transport layer unavailable — "
...
int ActionCommandSession::fire(const std::vector<ActionCommandTarget>& targets, uint32_t groupKey,
                               uint32_t groupMask) {
    if (targets.empty() || !d->gigeTl) {
        return 0;
    }
