...
static const char* k_level_labels[] = {"Trace", "Debug", "Info", "Warning", "Error", "Critical"};
...
LoggerPanelW::LoggerPanelW(QWidget* parent) : QWidget(parent), d(std::make_unique<Impl>()) {
    ...
    connect(&Logger::instance(), &Logger::entry_added, this, &LoggerPanelW::on_entry_added,
            Qt::QueuedConnection);
}
...
void LoggerPanelW::build_toolbar(QVBoxLayout* parent) {
    ...
    for (const auto* lbl : k_level_labels) {
        d->levelFilter->addItem(lbl);
    }
    d->levelFilter->setCurrentIndex(static_cast<int>(LogLevel::Error));
    d->levelFilter->setFixedWidth(90);
    connect(d->levelFilter, qOverload<int>(&QComboBox::currentIndexChanged), this,
            [this](int idx) { d->minLevel = idx; });
...
void LoggerPanelW::on_entry_added(int level, QString timestamp, QString location, QString message) {
    if (level < d->minLevel) {
        return;
    }
...
void LoggerPanelW::append_entry(int level, const QString& timestamp, const QString& location,
    ...
    if (d->autoScroll->isChecked()) {
        d->view->verticalScrollBar()->setValue(d->view->verticalScrollBar()->maximum());
    }
}
