...
//
// Four plugins today, selected via a combo box: Pose (YOLOv8) — a
...
// of each session's audio, shown as a playback-synced transcript table; and
...
// a per-blendshape score-over-time plot. AnalysisManager
// (analysis/analysis_manager.hpp) runs each plugin's script through the same
// shared subprocess queue. For Pose, the metrics plot can show raw Position
// (x/y) or, entirely computed client-side from the already-loaded result (no
// extra Python run needed — see analysis/pose_kinematics.hpp), derived
// Speed/Acceleration with optional smoothing and an optional manual px-to-mm
// scale.
