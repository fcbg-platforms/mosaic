...
class AnnotationBarW : public QWidget {
    Q_OBJECT
   public:
    explicit AnnotationBarW(QWidget* parent = nullptr) : QWidget(parent) {
        setFixedHeight(22);
   ...
   protected:
    void paintEvent(QPaintEvent*) override {
        ...
        for (const auto& ann : annotations_) {
            ...
            p.fillRect(x, 2, std::max(3, static_cast<int>(width() * 0.015)), height() - 4,
                       col.darker(120));
            p.setPen(col);
            p.drawRect(x, 2, std::max(3, static_cast<int>(width() * 0.015)), height() - 5);
        }

        // Cursor
        if (posMs_ >= 0 && durationMs_ > 0) {
            const int cx = static_cast<int>(static_cast<double>(posMs_) / durationMs_ * width());
    ...
    void mousePressEvent(QMouseEvent* e) override {
        ...
        const int64_t pos =
            static_cast<int64_t>(static_cast<double>(e->pos().x()) / width() * durationMs_);
...
class CameraSlotW : public QWidget {
   public:
    explicit CameraSlotW(int camIdx, QWidget* parent = nullptr) : QWidget(parent), camIdx_(camIdx) {
        ...
        liveDot_->setStyleSheet("QLabel{background:#554422;border-radius:4px;}");
    }
    ...
    void set_stats(double fpsActual, double coveragePct) {
        ...
        statsBadge_->setStyleSheet(
            QString("QLabel{background:#08081a;border:1px solid #1e1e36;"
                ...
                .arg(col.name()));
        statsBadge_->setText(QString("%.1f fps  %.0f%%").arg(fpsActual).arg(coveragePct));
        statsBadge_->adjustSize();
        reposition_badges();
    }
    ...
    void update_dot(bool live) {
        liveDot_->setStyleSheet(live ? "QLabel{background:#33cc66;border-radius:4px;}"
...
SessionPlayerW::SessionPlayerW(const SessionInfo& session, QWidget* parent)
    : QDialog(parent, Qt::Window), d(std::make_unique<Impl>()) {
    d->session = session;
...
void SessionPlayerW::build_ui() {
    ...
    auto* titleLbl =
        new QLabel(QString("▶  %1   <span style='color:#44446a;font-size:10px;'>%2</span>")
                       .arg(d->session.name)
    ...
    if (d->manifest.is_valid()) {
        ...
        for (int c = 0; c < d->manifest.camera_count(); ++c) {
            if (d->manifest.camera_info(c).coveragePct >= 50.0) {
                ++goodCams;
            }
        }
        d->qualityLabel->setText(QString("Sync: %1 / %2 cameras  ≥50% coverage")
                                     .arg(goodCams)
                                     .arg(d->manifest.camera_count()));
    }
    header->addWidget(d->qualityLabel);
    root->addLayout(header);

    // ── Camera grid ────────────────────────────────────────────────────────
    const int nCams = d->manifest.is_valid() ? d->manifest.camera_count() : d->session.cameraCount;
    const int gridCols = (nCams <= 2) ? nCams : (nCams <= 4) ? 2 : (nCams <= 6) ? 3 : 4;
    ...
    connect(d->annotBar, &AnnotationBarW::seek_requested, this,
            [this](int64_t posMs) { seek_all(posMs); });
    ...
    connect(d->scrubber, &QSlider::valueChanged, this, &SessionPlayerW::on_scrub_moved);

    d->durationLabel = new QLabel(SessionInfo::ms_to_hms(d->durationMs) + ".000");
    ...
    connect(d->stepBackBtn, &QToolButton::clicked, this, [this] { on_step(-1); });

    d->playBtn = new QPushButton("▶  Play");
    d->playBtn->setMinimumWidth(90);
    d->playBtn->setShortcut(Qt::Key_Space);
    connect(d->playBtn, &QPushButton::clicked, this, &SessionPlayerW::on_play_pause);

    d->stepFwdBtn = new QToolButton;
    d->stepFwdBtn->setText("▶");
    d->stepFwdBtn->setToolTip("Step forward one frame (→)");
    connect(d->stepFwdBtn, &QToolButton::clicked, this, [this] { on_step(+1); });

    d->stopBtn = new QPushButton("■  Stop");
    connect(d->stopBtn, &QPushButton::clicked, this, &SessionPlayerW::on_stop);
    ...
    connect(d->speedBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &SessionPlayerW::on_speed_changed);
    ...
    connect(d->volSlider, &QSlider::valueChanged, this, &SessionPlayerW::on_volume_changed);
...
void SessionPlayerW::setup_players() {
    ...
    for (int i = 0; i < nCams; ++i) {
        ...
        const int camIdx = i;
        connect(u.sink, &QVideoSink::videoFrameChanged, this,
                [this, camIdx](const QVideoFrame& vf) {
...
void SessionPlayerW::on_step(int direction) {
    ...
    const int64_t stepMs = static_cast<int64_t>(std::round(1000.0 / d->manifest.master_fps()));
    const int64_t newPos = std::clamp(d->masterMs + direction * stepMs, 0LL, d->durationMs);
    seek_all(newPos);
}
...
void SessionPlayerW::on_speed_changed(int index) {
    ...
    for (auto& u : d->units) {
        if (u.player) {
            u.player->setPlaybackRate(d->rate);
        }
    }
    if (d->audioPlayer) {
        d->audioPlayer->setPlaybackRate(d->rate);
    }
}
...
void SessionPlayerW::update_time_display(int64_t masterMs) {
    ...
    d->timeLabel->setText(QString("%1:%2:%3.%4")
                              .arg(h, 2, 10, QChar('0'))
                              .arg(m, 2, 10, QChar('0'))
                              .arg(s, 2, 10, QChar('0'))
                              .arg(ms, 3, 10, QChar('0')));
...
void SessionPlayerW::keyPressEvent(QKeyEvent* e) {
    switch (e->key()) {
        ...
        case Qt::Key_BracketLeft: {
            ...
            if (idx > 0) {
                d->speedBox->setCurrentIndex(idx - 1);
            }
            break;
        }
        case Qt::Key_BracketRight: {
            int idx = d->speedBox->currentIndex();
            if (idx < d->speedBox->count() - 1) {
                d->speedBox->setCurrentIndex(idx + 1);
            }
            break;
        }
...
void SessionPlayerW::resizeEvent(QResizeEvent* e) { QDialog::resizeEvent(e); }
