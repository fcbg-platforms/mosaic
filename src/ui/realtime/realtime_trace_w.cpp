...
RealtimeTraceW::RealtimeTraceW(QWidget* parent) : QWidget(parent), d(std::make_unique<Impl>()) {
...
void RealtimeTraceW::paintEvent(QPaintEvent*) {
    ...
    const QRectF plot(rc.left() + kMarginL, rc.top() + kMarginT, rc.width() - kMarginL - kMarginR,
    ...
    for (const double frac : {0.0, 0.5, 1.0}) {
        const double y = plot.bottom() - frac * plot.height();
        p.drawText(QRectF(rc.left(), y - 7, kMarginL - 4, 14), Qt::AlignRight | Qt::AlignVCenter,
                   QString::number(frac, 'f', 1));
    }
    ...
    const auto to_widget = [&](const Sample& s, double value) {
        const double tFrac =
            1.0 - static_cast<double>(nowMs - s.ms) / kWindowMs; // 0=left(oldest), 1=right(now)
        return QPointF(plot.left() + tFrac * plot.width(), plot.bottom() - value * plot.height());
    };
