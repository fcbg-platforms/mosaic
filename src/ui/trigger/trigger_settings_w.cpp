...
TriggerSettingsW::TriggerSettingsW(TriggerSettings& settings, TriggerManager* manager,
                                   QWidget* parent)
    : QWidget(parent), m_settings(settings), m_manager(manager), d(std::make_unique<Impl>()) {
    ...
    if (m_manager) {
        connect(m_manager, &TriggerManager::event_received, this,
                &TriggerSettingsW::on_event_received, Qt::QueuedConnection);
    }
}
...
void TriggerSettingsW::make_keyboard_card(int index) {
    ...
    connect(card, &KeyboardCardW::remove_requested, this,
            &TriggerSettingsW::remove_keyboard_trigger);

    // Connect the matching KeyboardTrigger's counter to the card's display.
    if (m_manager) {
        if (auto* kt = qobject_cast<KeyboardTrigger*>(m_manager->keyboard_trigger_at(index))) {
            connect(kt, &KeyboardTrigger::count_changed, card, &KeyboardCardW::on_count_changed,
                    Qt::QueuedConnection);
        }
    }
...
void TriggerSettingsW::build_serial_section(QVBoxLayout* parent) {
    ...
    addBtn->setFixedHeight(26);
    connect(addBtn, &QPushButton::clicked, this, [this] { add_serial_trigger({}); });
    headerRow->addWidget(addBtn);
    parent->addLayout(headerRow);
...
void TriggerSettingsW::make_serial_card(int index) {
    ...
    connect(card, &SerialCardW::remove_requested, this, &TriggerSettingsW::remove_serial_trigger);
...
void TriggerSettingsW::remove_serial_trigger(int index) {
    if (index < 0 || index >= d->serialCards.size()) {
        return;
    }
...
void TriggerSettingsW::build_parallel_port_section(QVBoxLayout* parent) {
    ...
    addBtn->setFixedHeight(26);
    connect(addBtn, &QPushButton::clicked, this, [this] { add_parallel_port({}); });
    headerRow->addWidget(addBtn);
    parent->addLayout(headerRow);
...
void TriggerSettingsW::make_parallel_port_card(int index) {
    ...
    connect(card, &ParallelPortCardW::remove_requested, this,
            &TriggerSettingsW::remove_parallel_port);
...
void TriggerSettingsW::remove_parallel_port(int index) {
    if (index < 0 || index >= d->parallelCards.size()) {
        return;
    }
...
void TriggerSettingsW::on_event_received(const TriggerEvent& event) {
    ...
    const QString line = QString(
                             ...
                             .arg(timeStr, color, event.source.toUpper().leftJustified(3, ' '),
