...
static std::array<double, N> json_to_array(const QJsonArray& ja, const std::array<double, N>& def) {
    if (static_cast<std::size_t>(ja.size()) != N) {
        return def;
    }
    std::array<double, N> out;
    for (std::size_t i = 0; i < N; ++i) {
        out[i] = ja[static_cast<int>(i)].toDouble();
    }
    return out;
}
...
std::optional<CalibrationData> CalibrationData::from_json(const QJsonObject& o) {
    CalibrationData cal;
    if (o.contains("calibrated")) {
        cal.calibrated = o["calibrated"].toBool();
    }
    if (o.contains("rms_error")) {
        cal.rmsError = o["rms_error"].toDouble(-1.0);
    }
    if (o.contains("camera_matrix")) {
        cal.cameraMatrix = json_to_array(o["camera_matrix"].toArray(), cal.cameraMatrix);
    }
    if (o.contains("dist_coeffs")) {
        cal.distCoeffs = json_to_array(o["dist_coeffs"].toArray(), cal.distCoeffs);
    }
    if (o.contains("extrinsic_rt")) {
        cal.extrinsicRt = json_to_array(o["extrinsic_rt"].toArray(), cal.extrinsicRt);
    }
...
std::optional<CameraParameters> CameraParameters::from_json(const QJsonObject& o) {
    ...
    if (o.contains("exposure_time_us"))
        c.exposureTimeUs = o["exposure_time_us"].toDouble(c.exposureTimeUs);
    if (o.contains("exposure_auto_lower"))
        c.exposureAutoLowerUs = o["exposure_auto_lower"].toDouble(c.exposureAutoLowerUs);
    if (o.contains("exposure_auto_upper"))
        c.exposureAutoUpperUs = o["exposure_auto_upper"].toDouble(c.exposureAutoUpperUs);
    ...
    if (o.contains("gain_auto_lower"))
        c.gainAutoLowerDb = o["gain_auto_lower"].toDouble(c.gainAutoLowerDb);
    if (o.contains("gain_auto_upper"))
        c.gainAutoUpperDb = o["gain_auto_upper"].toDouble(c.gainAutoUpperDb);
    ...
    if (o.contains("balance_white_auto"))
        c.balanceWhiteAuto = o["balance_white_auto"].toString(c.balanceWhiteAuto);
    if (o.contains("balance_ratio_red"))
        c.balanceRatioRed = o["balance_ratio_red"].toDouble(c.balanceRatioRed);
    if (o.contains("balance_ratio_blue"))
        c.balanceRatioBlue = o["balance_ratio_blue"].toDouble(c.balanceRatioBlue);
    ...
    if (o.contains("auto_target_brightness"))
        c.autoTargetBrightness = o["auto_target_brightness"].toDouble(c.autoTargetBrightness);
    if (o.contains("digital_shift")) c.digitalShift = o["digital_shift"].toInt(c.digitalShift);
    if (o.contains("hw_trigger_enabled"))
        c.hwTriggerEnabled = o["hw_trigger_enabled"].toBool(c.hwTriggerEnabled);
    if (o.contains("hw_trigger_source"))
        c.hwTriggerSource = o["hw_trigger_source"].toString(c.hwTriggerSource);
    if (o.contains("hw_trigger_delay_us"))
        c.hwTriggerDelayUs = o["hw_trigger_delay_us"].toDouble(c.hwTriggerDelayUs);
    if (o.contains("live_analysis_enabled"))
        c.liveAnalysisEnabled = o["live_analysis_enabled"].toBool(c.liveAnalysisEnabled);
...
QJsonObject MicrophoneParameters::to_json() const {
    return {
        {"device_id", deviceId}, {"name", friendlyName},      {"sample_rate", sampleRate},
        {"channels", channels},  {"buffer_size", bufferSize},
    };
}
...
std::optional<ParallelPortConfig> ParallelPortConfig::from_json(const QJsonObject& o) {
    ...
    if (o.contains("port_address")) {
        c.portAddress = o["port_address"].toString(c.portAddress);
    }
    if (o.contains("poll_rate_ms")) {
        c.pollRateMs = o["poll_rate_ms"].toInt(c.pollRateMs);
    }
    if (o.contains("enabled")) {
        c.enabled = o["enabled"].toBool(c.enabled);
    }
    if (o.contains("invert_logic")) {
        c.invertLogic = o["invert_logic"].toBool(c.invertLogic);
    }
    if (o.contains("action")) {
        c.action = trigger_action_from_label(o["action"].toString());
    }
...
QJsonObject TriggerSettings::to_json() const {
    ...
    for (const auto& k : keyboardTriggers) {
        keys.append(k.to_json());
    }
    for (const auto& s : serialTriggers) {
        serials.append(s.to_json());
    }
    for (const auto& p : parallelPorts) {
        ports.append(p.to_json());
    }
    return {
...
std::optional<TriggerSettings> TriggerSettings::from_json(const QJsonObject& o) {
    TriggerSettings s;
    if (o.contains("receive_enabled")) {
        s.receiveEnabled = o["receive_enabled"].toBool(s.receiveEnabled);
    }
    ...
    if (o.contains("serial_triggers")) {
        for (const auto& v : o["serial_triggers"].toArray()) {
            if (auto c = SerialTriggerConfig::from_json(v.toObject())) {
                s.serialTriggers.push_back(std::move(*c));
            }
        }
    }
    if (o.contains("parallel_ports")) {
        for (const auto& v : o["parallel_ports"].toArray()) {
            if (auto c = ParallelPortConfig::from_json(v.toObject())) {
                s.parallelPorts.push_back(std::move(*c));
            }
        }
    }
    return s;
...
std::optional<RecordSettings> RecordSettings::from_json(const QJsonObject& o) {
    ...
    if (o.contains("video_basename"))
        s.videoBasename = o["video_basename"].toString(s.videoBasename);
    if (o.contains("audio_basename"))
        s.audioBasename = o["audio_basename"].toString(s.audioBasename);
    if (o.contains("trigger_basename"))
        s.triggerBasename = o["trigger_basename"].toString(s.triggerBasename);
    ...
    if (o.contains("timestamp_format"))
        s.timestampFormat = o["timestamp_format"].toString(s.timestampFormat);
...
std::optional<RoomSettings> RoomSettings::from_json(const QJsonObject& o) {
    RoomSettings s;
    if (o.contains("plane_point")) {
        s.planePoint = json_to_array(o["plane_point"].toArray(), s.planePoint);
    }
    if (o.contains("plane_normal")) {
        s.planeNormal = json_to_array(o["plane_normal"].toArray(), s.planeNormal);
    }
...
std::optional<RealtimeSettings> RealtimeSettings::from_json(const QJsonObject& o) {
    RealtimeSettings s;
    if (o.contains("enabled")) s.enabled = o["enabled"].toBool(s.enabled);
    if (o.contains("auto_pause_during_recording"))
        s.autoPauseDuringRecording =
            o["auto_pause_during_recording"].toBool(s.autoPauseDuringRecording);
...
bool AppSettings::save(const QString& path) const {
    ...
    if (!QDir().mkpath(QFileInfo(path).absolutePath())) {
        log_error(
            QString("Cannot create config directory: %1").arg(QFileInfo(path).absolutePath()));
        return false;
    }
